...
- For the demo path: an empty checkout of [multi-agent-lab](https://github.com/aion-lumen/multi-agent-lab) (Python 3.11+).
...
|---|---|---|
| `~/.folio/folio-demo.db` | folio | 2 worker_runs, 21 worker_run_logs, 15 validator_opinions |
| `<multi-agent>/state/feedback-demo.db` | multi-agent-lab | 40 demo mails (uid 90001–90040) |

The demo server forces `FOLIO_DB_PATH`, `FEEDBACK_DB_PATH`, and `VAULT_PATH` to demo paths; your real `~/.folio` is untouched.

The seed step lives in multi-agent-lab — see [its quickstart](https://github.com/aion-lumen/multi-agent-lab/blob/main/docs/quickstart.md) and the [README's Demo (full stack)](../README.md#demo-full-stack) walkthrough for the ordered cold-start. A cold-start machine with no `~/.folio` and no IMAP credentials works — `init_demo_dbs.sh` bootstraps the schemas from static SQL dumps when no real DBs exist.

## Verifying the demo

After `make demo` + `bash scripts/demo-server.sh`:

- `http://localhost:5174/pipeline` — Worker / Validator cards + Verlauf history with seeded runs
