...
Collector deployment is configured separately; Folio consumes the resulting aggregates read-only.
...
Open `http://127.0.0.1:4173` and follow the Setup Wizard. On first launch you can connect an existing vault, start from a demo vault, or create one from scratch.
...
Optional source adapters and local model runtimes require their own configuration; see
[source configuration](docs/source-configuration.md). Before upgrading an existing installation,
follow [upgrade and rollback](docs/preview-0.6.md#upgrade-and-rollback). Secure document extraction
uses the documented macOS tools.
...
### Mail in everyday use

Mail is Folio's main flow into an up-to-date memory and campaign. The queue puts category,
actionability and model judgements next to each message. The pipeline makes progress and
disagreement visible; the run history explains why a case was held for review.

These established demo captures show the mail workflow. They come from the June 2026 preview;
model labels and some surrounding controls reflect that version, not a new benchmark.

<p align="center">
  <a href="docs/screenshots/mail/01-mail-queue-20260611.png"><img src="docs/screenshots/mail/01-mail-queue-20260611.png" width="900" alt="Mail queue with categories, actionability and model assessments beside each fictional message" /></a>
  <br><sub><em>Mail queue — the message, its classification and the model assessments together.</em></sub>
</p>

<p align="center">
  <a href="docs/screenshots/mail/02-pipeline-idle-20260611.png"><img src="docs/screenshots/mail/02-pipeline-idle-20260611.png" width="900" alt="Mail pipeline showing import, worker, validator and subsequent processing in the earlier demo" /></a>
  <br><sub><em>Pipeline — each processing stage and its current state.</em></sub>
</p>

<details>
<summary>Model checks and detailed run history</summary>

![Model validator in the June demo](docs/screenshots/mail/03-pipeline-validator-20260611.png)

![Run history and reasons a case needs review](docs/screenshots/mail/05-verlauf-detail-20260611.png)

</details>

[Mail screenshots and provenance](docs/screenshots/mail/README.md) · [Detailed pipeline walkthrough](https://aion-lumen.ch/multi-agent/)
...
<p align="center">
  <a href="docs/screenshots/v0.6.0-preview.1/ledger-dashboard-complete.jpg"><img src="docs/screenshots/v0.6.0-preview.1/ledger-dashboard-complete.jpg" width="900" alt="Complete Ledger preview: assets, monthly income and expenses, cash flow, categories and period slider; synthetic data" /></a>
  <br><sub><em>One complete Ledger view, including the period slider. Synthetic data; click for the 1920 × 1365 image.</em></sub>
</p>
...
- **Mobile review** — continue daily-use validation of the approval and correction surface
  alongside the desktop workflow

These items remain development direction, not promises of the current release. See
